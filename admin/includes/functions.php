...
function login_user($username, $password)
{
    ...
    if (password_verify($password, $db_user_password)) {
        ...
        redirect("../login.php?msg=danger&msg2=Username or Password you entered is wrong");
    }
}
...
function delete_slider()
{
    ...
    if (isset($_GET['delete'])) {
        ...
        if (isset($_SESSION['user_role'])) {
            if ($_SESSION['user_role'] == 'admin') {
                $the_slider_id = mysqli_real_escape_string($connection, $the_slider_id);

                $query = "DELETE FROM sliders WHERE slider_id = {$the_slider_id}";
                $delete_query = mysqli_query($connection, $query);

                if ($delete_query) {
                    echo "<div class='alert alert-info alert-dismissable'><strong>Seccess!</strong> Slider Deleted</div>";
                } else {
                    die('Query Failed' . mysqli_error($connection));
                }
            }
...
function delete_main_contents()
{
    global $connection;
    if (isset($_GET['delete'])) {
        $the_contents_id = $_GET['delete'];
        if (isset($_SESSION['user_role'])) {
            if ($_SESSION['user_role'] == 'admin') {
                $the_contents_id = mysqli_real_escape_string($connection, $the_contents_id);

                $query = "DELETE FROM main_contents WHERE main_content_id = {$the_contents_id}";
                $delete_query = mysqli_query($connection, $query);

                if ($delete_query) {
                    echo "<div class='alert alert-info alert-dismissable'><strong>Seccess!</strong>  Deleted</div>";
                } else {
                    die('Query Failed' . mysqli_error($connection));
                }
            }
    ...
    if (isset($_GET['delete'])) {
        ...
        if (isset($_SESSION['user_role'])) {
            if ($_SESSION['user_role'] == 'admin') {
                ...
                $query = "DELETE FROM side_contents WHERE side_id = {$id}";
                $delete_query = mysqli_query($connection, $query);
    ...
    if (isset($_GET['delete'])) {
        ...
        if (isset($_SESSION['user_role'])) {
            if ($_SESSION['user_role'] == 'admin') {
                ...
                $query = "DELETE FROM admission WHERE admission_id = {$id}";
                $delete_query = mysqli_query($connection, $query);

                if ($delete_query) {
                    echo "<div class='alert alert-info alert-dismissable'><strong>Seccessfully!</strong>  Deleted</div>";
                } else {
                    die('Query Failed' . mysqli_error($connection));
                }
            }
    ...
    if (isset($_GET['delete'])) {
        ...
        if (isset($_SESSION['user_role'])) {
            if ($_SESSION['user_role'] == 'admin') {
                ...
                $query = "DELETE FROM about_us WHERE about_us_id = {$id}";
                $delete_query = mysqli_query($connection, $query);
